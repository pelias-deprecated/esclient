...
var BufferedBulkStream = function( client, options )
{
  ...
  this.buffer = [];
  this.flooding = false;
  this.options = options || { buffer: 200, timeout: 1000, throttle: 100 }
  ...
  this.client.bulk( { body: writeBuffer }, function( err, resp ){
    ...
    if( err ){
      this.stats.error += writeBufferTotal;
       return this.emit( 'error', new IndexError( err || 'bulk index error' ));
    }

    // response does not contain items
    if( !resp || !resp.items ){
      this.stats.error += writeBufferTotal; // consider them as failed
      return this.emit( 'error', new IndexError( 'invalid resp from es bulk index operation' ));
    }
